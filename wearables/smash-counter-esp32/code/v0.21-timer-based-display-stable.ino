// Smash Counter ESP32 v0.21 — Timer Based Display Stable
// Lishan, 2 June 2026
//
// Problem with v0.20:
// Still crashing on hard smash.
// Drawing display immediately on smash detection
// was conflicting with WiFi task even on separate core.
//
// Fix — timer based display updates:
// Remove all drawNumber(), drawBest(), drawSPM() calls
// from inside the smash detection block.
// Instead: update display every 500ms on a timer.
// Smash detection only updates data variables.
// Display reads variables and updates on its own schedule.
// No conflict between sensor events and display drawing.
//
// Result: stable operation during hard smashes. ✅
//
// Button behaviour:
// Short press BOOT = reset counter
// Long press BOOT 3 seconds = deep sleep
// Press BOOT once to wake from sleep
//
// Device is now fully wireless on battery. ✅
// Battery connected without soldering — Papa's solution.
...
void loop() {
  ...
  // Update display every 500ms — no conflict with WiFi
