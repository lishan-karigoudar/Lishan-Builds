// Smash Counter ESP32 v0.15 — Session Timer
// Lishan, 27 May 2026
//
// New feature: Session Timer
// Shows HH:MM:SS counting up from when session starts.
// Timer resets when BOOT button pressed.
// Updates every 1 second — not every loop iteration.
// Uses millis() — no extra library needed.
//
// Important correction from schematic review:
// GPIO4 = IMU_INT1 — NOT free ⚠️
// GPIO5 = TP_INT (touch) — NOT free ⚠️
// These were listed in roadmap as free — corrected here.
// Vibration motor will use GPIO15 instead — confirmed free.
//
// Free GPIOs confirmed from schematic:
// GPIO15, GPIO16, GPIO17, GPIO18, GPIO21, GPIO33
//
// Screen layout now:
// Top: SMASH COUNTER title
// Centre: large smash count number
...
void drawTimer() {
  ...
  int secs = elapsed % 60;
  tft.fillRect(45, 195, 140, 16, 0x0000);
  tft.setTextSize(1);
  tft.setTextColor(0x07FF);
  tft.setCursor(48, 197);
...
void drawBest() {
  yield();
  tft.fillRect(40, 175, 120, 12, 0x0000);
  tft.setTextSize(1);
