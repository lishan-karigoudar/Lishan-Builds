// Smash Counter ESP32 v0.32 — Charging Symbol Detection
// Lishan, 15 June 2026
//
// What happened:
// Battery display now shows charging symbol when USB plugged in.
// Detects charging by reading voltage — when above 4.15V, charger is active.
//
// Major change:
...
//
// How it works:
// Reads raw ADC value from BAT_PIN.
// Calculates voltage using divider formula: (160K + 82K) / 82K = 3.0.
// Compares voltage: if > 4.15V then charging, else discharging.
// Displays appropriate indicator on screen.
//
// Testing results:
// Charging detection works when USB plugged in ✓
// Symbol appears immediately ✓
// Battery percentage accurate ✓
// Color thresholds working correctly ✓
// No interference with smash detection ✓
//
// Session context:
// Discussed how to show charging like a phone does.
// VBUS detection not directly exposed on this board.
// Voltage threshold method proved reliable.
...
float getBatteryVoltage() {
  int raw = analogRead(BAT_PIN);
  return (raw / 4095.0) * 3.3 * ((160.0 + 82.0) / 82.0);
}

bool isCharging() {
  return getBatteryVoltage() > 4.15;
}
...
void buzz(int times) {
  buzzCount = times;
  buzzEndTime = millis() + (times * 150);
  buzzCompleted = false;
}

void handleBuzz() {
  if (buzzCompleted) return;
  
  unsigned long elapsed = millis() - (buzzEndTime - (buzzCount * 150));
  
  if (elapsed < (buzzCount * 150)) {
...
void handleRoot() {
  ...
  html += "<div class='bat'>🔋 " + String(bat) + "% <span class='charging'>" + chargingStatus + "</span></div>";
  if (goalAchieved) {
...
void drawBattery() {
  ...
  tft.fillRect(155, 22, 40, 16, 0x0000);
  tft.drawRect(155, 22, 28, 14, 0xFFFF);
  tft.fillRect(183, 26, 3, 6, 0xFFFF);
  
  uint16_t color;
  if (pct >= 70) color = 0x07E0;
  else if (pct >= 40) color = 0xFD20;
  else color = 0xF800;
...
void drawModeSelectionScreen() {
  ...
  tft.fillRoundRect(15, 15, 60, 25, 6, 0x4208);
  tft.setTextColor(0xFFFF);
  tft.setTextSize(1);
  tft.setCursor(30, 23);
  tft.print("EXIT");
  ...
  tft.fillRoundRect(35, 85, 170, 35, 8, 0x03E0);
  tft.setTextColor(0x0000);
  tft.setTextSize(2);
  tft.setCursor(75, 95);
  tft.print("EASY");
  tft.setTextSize(1);
  tft.setCursor(110, 112);
  tft.print("7 SPM");
  tft.fillRoundRect(35, 130, 170, 35, 8, 0xFD20);
  tft.setTextColor(0x0000);
  tft.setTextSize(2);
  tft.setCursor(60, 140);
  tft.print("MEDIUM");
  tft.setTextSize(1);
  tft.setCursor(110, 157);
  tft.print("10 SPM");
  tft.fillRoundRect(35, 175, 170, 35, 8, 0xF800);
  tft.setTextColor(0xFFFF);
  tft.setTextSize(2);
  tft.setCursor(75, 185);
  tft.print("HARD");
  tft.setTextSize(1);
  tft.setCursor(110, 202);
  tft.print("15 SPM");
}
...
void loop() {
  ...
  if (touch.available()) {
    ...
    if (!showModeScreen) {
      ...
      if (tx >= 15 && tx <= 75 && ty >= 15 && ty <= 40) {
        showModeScreen = false;
        drawInitialScreen();
      } else if (ty >= 85 && ty <= 120) {
        currentMode = 0;
        showModeScreen = false;
        prefs.begin("smash", false);
