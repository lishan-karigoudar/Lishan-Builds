// Smash Counter ESP32 v0.25 — Touch Mode Selection
// Lishan, 9 June 2026
//
...
//
// New features:
// 1. Touch screen mode selection — swipe right to open
// 2. Easy / Medium / Hard training modes
// 3. SPM target shown on screen per mode
// 4. Goal achieved / target pending indicator
// 5. Mode saved to flash — remembers after restart
//
// Touch chip: CST816S on I2C address 0x15
// Shared I2C bus with IMU (SDA=GPIO6, SCL=GPIO7)
// Touch INT = GPIO5, Touch RST = GPIO13
// Library: CST816S by fbiego — installed via Library Manager
//
// Mode targets:
// Easy   = 7 SPM  (green)
// Medium = 10 SPM (orange)
// Hard   = 15 SPM (red)
//
// How it works:
// Main screen: shows all stats + current mode indicator
// Swipe right: opens mode selection screen
// Tap Easy/Medium/Hard: selects mode, returns to main screen
// Tap Exit: returns without changing mode
// Mode saved in Preferences — survives deep sleep and restart
//
// Papa's reaction: "it is very nice" ✅
//
// Known issue: buzz() with delay() blocks WiFi task
// causing freeze and crash — fixed in v0.26/v0.27
