// Smash Counter ESP32 v0.27 — Exit Button Top + Freeze Pending
// Lishan, 9 June 2026
//
// Change: Exit button moved to top left of mode selection screen.
// Previous: Exit was at bottom — hard to reach mid-match.
// Now: Exit top left, current mode name top right.
// Easier to exit quickly during a badminton session.
//
// Mode selection screen layout:
// Top left: EXIT button (grey)
// Top right: current mode name (colour coded)
// Below: SELECT MODE title
// Then: EASY (green), MEDIUM (orange), HARD (red) buttons
//
// Known issue — CRASH:
// Device freezes for ~10 seconds then turns off.
// Root cause: buzz() function uses delay() inside it.
// delay() in main loop blocks Core 1.
// WiFi task on Core 0 cannot get data from Core 1.
// Watchdog timer kills the process after timeout.
//
// Fix needed:
// Replace buzz() with non-blocking version using millis().
// Schedule buzz events in a queue instead of blocking with delay().
// To be fixed next session.
//
// Session ended 9 June — "goodnight" — fix deferred to next weekend.
...
void buzz(int times, int duration) {
  // NOTE: This uses delay() — blocks main loop — causes freeze crash
  // Fix pending: replace with non-blocking millis() based buzz
